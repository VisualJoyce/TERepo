...
def read_cilin():
    ...
    """
    # TODO -- fix this path
    lines = open(os.path.join(os.path.dirname(REAL_PATH), "data/cilin.txt"), "r").read().strip().split("\n")
    semantic_dict = {}
    semantic_classes = {}
    for line in lines:
...
def read_confusion():
    confusion_dict = {}
    with open(os.path.join(os.path.dirname(REAL_PATH), "data/confusion_dict.txt"), "r", encoding="utf-8") as f:
        for line in f:
            li = line.rstrip('\n').split(" ")
