...
class FocalLoss1(nn.CrossEntropyLoss):
    """ Focal loss for classification tasks on imbalanced datasets """

    def __init__(self, gamma, alpha=None, ignore_index=-100, reduction='none', label_smoothing: float = 0.0):
    ...
    def extra_repr(self) -> str:
        return 'gamma={}, alpha={}, reduction={}'.format(
            self.gamma, self.weight, self.reduction
        )
...
class FocalLoss(nn.Module):
    ...
    def forward(self, input_, target):
        cross_entropy = self.ce_loss(input_, target)
        # Temporarily mask out ignore index to '0' for valid gather-indices input.
        # This won't contribute final loss as the cross_entropy contribution
        # for these would be zero.
        target = target * (target != self.ignore_index).long()
        input_prob = torch.gather(torch.softmax(input_, 1), 1, target.unsqueeze(1))
        loss = torch.pow(1 - input_prob, self.gamma) * cross_entropy
        return torch.mean(loss) if self.reduction == 'mean' else torch.sum(loss) if self.reduction == 'sum' else loss

    def extra_repr(self) -> str:
        return 'gamma={}, alpha={}, reduction={}'.format(
            self.gamma, self.alpha, self.reduction
        )
